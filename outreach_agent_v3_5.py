"""
Semi-Agentic Outreach System v3.5 — Theta-Native Research + People-First Outreach
=====================================================
CHANGES FROM v3.4:
  [1] Archetype system: simple 1-6 numbered patterns with visual zone bars
  [2] People finder: real names first, search strings as fallback (no more "Unknown")
  [3] Cache people results: 30-day cache per company (major cost saver)
  [4] Batch research: all 5 companies researched in ONE API call (not 5 separate calls)

THREE MODES:
  python outreach_agent_v3_5.py           → Monday: audit + hunt + plan
  python outreach_agent_v3_5.py --status  → Any day: quick check, no AI, no cost
  python outreach_agent_v3_5.py --friday  → Friday: upload JSON, CEO review, save learnings
...
class CostTracker:
    COSTS = {
        "hunt":              0.15,
        "research":          0.12,
        "research_batch":    0.20,   # one call for 5 companies (much cheaper than 5 × 0.12)
        "cached_research":   0.01,
        "cached_people":     0.00,   # people from cache = free
        "draft":             0.05,
        "followup_draft":    0.03,
        "theta":             0.02,
        "channels":          0.02,
    ...
    def projection(self, remaining_companies: int) -> str:
        ...
        return (
            ...
            f"\n  📊 Projected for {remaining_companies} more companies: ${projected:.2f}/company × {remaining_companies} = ${projected:.2f}"
...
def load_cached_people(company: str) -> list:
    """Load people from cache if within 30-day TTL. Free — zero API cost."""
    cached = load_cache(company)
    return cached.get("people", [])

def save_cached_people(company: str, people: list):
    """Cache found people for 30 days."""
    save_cache(company, {"people": people})

# ─────────────────────────────────────────────────────────────
# [IMPROVEMENT 1] ARCHETYPE SYSTEM — Simple 1-6, Visual Bars
# ─────────────────────────────────────────────────────────────

# Six plain-language archetypes mapped to zone patterns
VALID_ARCHETYPES = [
    "1. Core Heavy",    # Core dominant, little else — optimizing existing, no visible next curve
    "2. Edge Active",   # Building next S-curve, strong Edge, limited long-term
    "3. Beyond Funded", # Long-term bets funded, but may lack Core/Edge bridge
    "4. Balanced",      # Active across all three zones
    "5. Core + Edge",   # Strong present + next curve, no long-horizon bets
    "6. Theater Risk",  # Labs/announcements, little evidence of scaling
]

# Add your own here if needed — just extend the list with "7. Your Label"

def display_archetype(arch: str) -> str:
    """Strip number prefix for clean display: '1. Core Heavy' → 'Core Heavy'"""
    if arch and '.' in arch:
...
def determine_archetype(core: int, edge: int, beyond: int, theater: int) -> str:
    """
    Pure zone-score logic. No marketing names.
    Returns a numbered string so it sorts cleanly.
    ...
    if core >= 5 and edge >= 5 and beyond < 2:
        return "5. Core + Edge"  # Check before Edge Active — both core AND edge strong, no beyond
    if edge >= 5 and core >= 4 and beyond < 3:
        return "2. Edge Active"  # Edge dominant, core supporting
    if 4 <= core <= 7 and 4 <= edge <= 7 and 2 <= beyond <= 4:
        return "4. Balanced"
    return "4. Balanced"  # default

def get_focus_area(archetype: str) -> str:
    """For messaging angle — what tension is worth asking about."""
...
def display_theta_visual(company_name: str, theta: dict):
    """Visual zone bars — readable at a glance."""
...
# ─────────────────────────────────────────────────────────────
# THETA FRAMEWORK — Updated to use simple archetype system
# ─────────────────────────────────────────────────────────────
...
def theta_assess(research_text: str, company_name: str, industry: str,
    ...
    if override_archetype and any(override_archetype in a for a in VALID_ARCHETYPES):
        # Match partial (e.g. "Core Heavy" matches "1. Core Heavy")
...
# ─────────────────────────────────────────────────────────────
# THETA FRAMEWORK BRIEF (for prompts)
# ─────────────────────────────────────────────────────────────
...
Do NOT miss Beyond investments — companies like Nestlé have significant ones (microbiome, precision nutrition, cellular agriculture).
"""

# ─────────────────────────────────────────────────────────────
# [IMPROVEMENT 4] BATCH RESEARCH — all companies in ONE call
# ─────────────────────────────────────────────────────────────

def batch_research_companies(client, companies: list, cost: CostTracker,
                              cs_contexts: dict = None) -> dict:
    """
    Research all uncached companies in a SINGLE API call.
    Returns dict keyed by company name → {background, theta, was_cached}.
    """
    results        = {}
    to_research    = []
    ...
    for company in companies:
        ...
        if "background" in cached:
            results[name] = {
                ...
                "theta":      theta_assess(cached["background"], name, company.get("industry","")),
                "was_cached": True,
            }
    ...
    # Build batch prompt — ONE API call for all uncached companies
    print(f"\n  📡  Batch researching {len(to_research)} companies in one API call...")
...
1. CORE zone: Key Core initiatives (existing products, efficiency, optimization)
2. EDGE zone: Active Edge bets (new platforms, adjacent markets, digital transformation)
3. BEYOND zone: Beyond investments (transformational R&D, long-horizon bets) — be thorough, do NOT miss these
4. ARCHETYPE: Which of the 6 patterns fits? (1. Core Heavy / 2. Edge Active / 3. Beyond Funded / 4. Balanced / 5. Core + Edge / 6. Theater Risk)
5. BIGGEST GAP: Where is their portfolio weakest?
6. WHO OWNS BREAKTHROUGH: Which specific role owns Edge and Beyond innovation?

Keep each company under 300 words. Be specific — name actual programs.
...
"""
    ...
    try:
        ...
        raw  = r.content[0].text.strip()
        cost.charge("research_batch", note=f"{len(to_research)} companies")
        ...
        for company in to_research:
            ...
            for block in blocks[1:]:
                ...
                # Fuzzy match — company name might differ slightly
            ...
            if not background and blocks:
                # Fallback: use the block by position
        ...
        for company in to_research:
            name   = company.get("name","")
            ctx    = cs_contexts.get(name, {})
            result = research_company_single(client, company, cost, ctx)
            results[name] = result
        return results


def research_company_single(client, company: dict, cost: CostTracker,
                             cs_context: dict = None) -> dict:
    """Single-company research fallback (used only if batch fails)."""
    name     = company.get("name","")
...
Provide: CORE zone, EDGE zone, BEYOND zone (thorough — do not miss Beyond investments),
...
Under 350 words. Be specific."""
    ...
    try:
        ...
        save_cache(name, {"background": background})
        cost.charge("research", note=f"{name} (single)")
    except Exception as e:
...
def ask_case_study_context(company_name: str, industry: str) -> dict:
    """Ask Pam for her case study / articles. If URL given, fetch and read it."""
...
def extract_case_study_insights(case_text: str, company: str) -> dict:
    """Pull key insights from case study text — free, no API cost."""
    if not case_text:
        return {}
    tension_keywords = ["tension","dilemma","challenge","struggle","paradox","gap","risk"]
    key_sentences = []
    for sentence in re.split(r'[.!?]', case_text):
        s = sentence.strip()
        if s and any(k in s.lower() for k in tension_keywords):
            key_sentences.append(s)
    return {
        "tension_signals": key_sentences[:3],
        "char_count":      len(case_text),
    }

# ─────────────────────────────────────────────────────────────
# [IMPROVEMENT 2] FIND TARGET PEOPLE — Real names first
# [IMPROVEMENT 3] CACHE PEOPLE RESULTS
# ─────────────────────────────────────────────────────────────

def find_target_people(client, company: dict, research_bg: str, cost: CostTracker) -> list:
    """
    Find 3–5 people who own Edge/Beyond at this company.
    REAL NAMES when known. Search strings as fallback.
    Results cached for 30 days (free on subsequent runs).
    """
    name     = company.get("name","")
    industry = company.get("industry","")

    # [IMPROVEMENT 3] Check cache first — free
    cached_people = load_cached_people(name)
    if cached_people:
        print(f"  ✓ People loaded from cache (free — saved ${CostTracker.COSTS.get('research', 0.12):.2f})")
...
TITLE: [Exact current title as it appears on LinkedIn]
WHY: [One sentence — what specifically they own that's relevant to Theta]
LINKEDIN_SEARCH: [Precise search string, e.g.: "Nestlé" "Chief Innovation Officer" -intern]
CONFIDENCE: [High = verified real person / Medium = likely this role exists / Low = guessing]

Repeat for each person."""

    try:
        r = client.messages.create(model="claude-haiku-4-5-20251001", max_tokens=900,
                                    messages=[{"role":"user","content":prompt}])
        raw    = r.content[0].text.strip()
        people = []
        for block in raw.split("---PERSON---")[1:]:
            ...
            # Clean up: reject blank names, replace None/empty with "Search required"
            if not p.get("name") or p["name"].strip() in ["","Unknown","N/A"]:
                p["name"] = "Search required"
            if p.get("title"):
                people.append(p)

        cost.charge("research", note=f"{name} people (fresh)")

        # [IMPROVEMENT 3] Save to cache
        if people:
            save_cached_people(name, people)
            print(f"  ✓ {len(people)} people found and cached for next time")
...
def check_public_channels(client, person: dict, company_name: str, cost: CostTracker) -> dict:
    """Assess which public channel is easiest first touch for this person."""
...
# ─────────────────────────────────────────────────────────────
# DRAFT CONNECTION + POST-CONNECTION MESSAGES
# ─────────────────────────────────────────────────────────────
...
[message]""".format(channel=recommended)
        ...
        note_instruction = """IMPORTANT: Pam has NO connection notes available (free LinkedIn limit).
She will send a BLANK invite. This message is the FIRST thing this person reads from her.
...
# ─────────────────────────────────────────────────────────────
# [IMPROVEMENT 1b] ARCHETYPE CORRECTION LOOP — validated 1-6
# ─────────────────────────────────────────────────────────────
...
def prompt_archetype_correction(current_archetype: str, company_name: str,
                                 ...
                                 learnings: dict) -> dict:
    """
    Prompt Pam to correct the archetype. Always validates 1-6.
    Loops until valid input or cancel.
    """
...
def save_all(approved_new, followup_actions, warm, patterns, cost):
    ...
    if approved_new:
        export = {
            ...
            "source":     "semi-agentic-outreach-v3.5",
            "note":       (
...
Board-meeting level. No fluff."""
    ...
    print(f"\n  📋  Next Monday: python outreach_agent_v3_5.py\n")
...
def main():
    parser = argparse.ArgumentParser(description="Semi-Agentic Outreach System v3.5")
    parser.add_argument("--friday", action="store_true",
                        help="Friday review mode")
    parser.add_argument("--status", action="store_true",
                        help="Quick daily status check — no AI calls, free")
    ...
    # ── MONDAY MODE ──────────────────────────────────────────
    banner("🧠  SEMI-AGENTIC OUTREACH v3.5  |  Theta Framework")
    print(f"  {datetime.now().strftime('%A, %B %d %Y  %H:%M')}")
    print(f"  {day_greeting()}")
    print(f"\n  v3.5 improvements active:")
    print(f"    ✓ Archetype system: simple 1-6 with visual zone bars")
    print(f"    ✓ People finder: real names first, search strings as fallback")
    print(f"    ✓ People cache: 30-day cache per company (free on repeat runs)")
    print(f"    ✓ Batch research: all companies in one API call")
    ...
    if do_hunt == "y":
        # Ask once per session about LinkedIn notes
        ...
        if new_companies:
            ...
            # ── STEP A: Collect case study contexts for all companies first ──
            print(f"\n  First, let's collect your content for each company.")
            print(f"  This feeds into the Theta analysis so it's informed by your case studies.\n")
            cs_contexts = {}
            ...
            # ── STEP B: Batch research ALL companies in one API call ──
            print(f"\n  {cost.projection(len(new_companies))}")
            research_results = batch_research_companies(client, new_companies, cost, cs_contexts)

            print(f"\n  ✓ Research complete. Now reviewing companies one at a time.")
            ...
            for i, company in enumerate(new_companies, 1):
                ...
                # ── Display Theta visual ──
                section(f"THETA BRIEF — {cname}{cached_tag}")
                display_theta_visual(cname, theta)

                bg = research.get("background","")
                print(f"\n  Research summary:")
                for line in bg.splitlines()[:8]:
                    if line.strip(): print(f"    {line}")
                if len(bg.splitlines()) > 8:
                    print(f"    ... ({len(bg.splitlines())} lines total)")

                # ── Archetype correction with validated 1-6 loop ──
                correct_arch = ask("\n  Archetype correct?", ["y","n"])
                if correct_arch == "n":
                    theta = prompt_archetype_correction(
                        theta.get("archetype",""), cname, theta,
                        bg, company.get("industry",""), learnings
                    )
                ...
                # ── Find target people (with caching) ──
                print(f"\n  Finding who owns breakthrough innovation at {cname}...")
                people = find_target_people(client, company, bg, cost)
                ...
                for person in selected_people:
                    ...
                    channel_info = check_public_channels(client, person, cname, cost)
    ...
    print(f"  Any day → python outreach_agent_v3_5.py --status  (quick check)")
    print(f"  Fri  → Export tracker → python outreach_agent_v3_5.py --friday")
    print()
