...
def find_people_via_perplexity(api_key: str, company_name: str, industry: str,
    ...
    """
    [v3.9] Use Perplexity sonar (live web) to find real, current people.
...
def find_target_people(ds_client, claude_client, company: dict, research_bg: str,
    ...
    """
    [v3.9] Four-tier people lookup. Returns (people_list, source, verified_entry).

    Priority chain:
      1. Your case study HTML → extract named people (free, most accurate)
      2. Verified cache in learnings.json (free, confirmed by you, 90-day TTL)
      3. Perplexity sonar → live web search (cheap, current names with sources)
      4. DeepSeek/Claude → last resort, LOW CONFIDENCE warning shown

    DeepSeek is never the first choice for people anymore.
    """
    name = company.get("name", "")
    ...
    # ── TIER 3: Old file cache (treat as unverified) ──────────
    cached_people = load_cached_people(name)
    if cached_people:
        # Only use if NOT from DeepSeek (check confidence flags)
        verified_cached = [p for p in cached_people if p.get("confidence") == "Verified"
                           or p.get("_from_case_study") or p.get("_from_perplexity")]
        if verified_cached:
            print(f"  ✓ {len(verified_cached)} verified people from file cache")
            cost.charge("cached_people", note=f"{name} (file cache, verified)")
            return verified_cached, "verified_cache", None

    # ── TIER 4: Perplexity live search ────────────────────────
    if pplx_key:
        print(f"  🔍  Searching live web via Perplexity for current {name} leaders...")
        people, citations = find_people_via_perplexity(
            pplx_key, name, company.get("industry", ""), research_bg, cost)
        if people:
            # Mark as perplexity-sourced
            for p in people:
                p["_from_perplexity"] = True
            save_cached_people(name, people)
            print(f"  ✓ {len(people)} people found via Perplexity live search — cached")
...
IMPORTANT: DeepSeek has NO internet access. Only provide names you are very
confident about. Say "Search required" if uncertain — do NOT invent names.

TARGET ROLES: Chief Innovation Officer, Chief Strategy Officer, CTO, Head of R&D,
Head of New Ventures, VP Digital Transformation.
...
def people_selection_loop(people: list, source: str, verified_entry: dict | None,
    ...
    if source in ("verified_cache", "case_study", "manual") and verified_entry:
        ...
        print(f"\n  ✅  EXTRACTED FROM YOUR CASE STUDY — these people are named in your own research")
    elif source == "perplexity":
        print(f"\n  🔍  SOURCED VIA PERPLEXITY LIVE SEARCH — current web results, verify on LinkedIn")
    else:
...
def main():
    ...
    # ── MONDAY MODE ──────────────────────────────────────────
    banner("🧠  SEMI-AGENTIC OUTREACH v3.9  |  Theta Framework")
    print(f"  {datetime.now().strftime('%A, %B %d %Y  %H:%M')}")
    print(f"  {day_greeting()}")
    print(f"\n  v3.9 active:")
    print(f"    ✓ Case study registry — {len(KNOWN_CASE_STUDIES)} studies auto-applied, no prompts")
    print(f"    ✓ People: case study extraction → verified cache → Perplexity live → DeepSeek last")
    print(f"    ✓ Related case study suggestions when no exact match")
    print(f"    ✓ Case study backlog — track what to research next")
