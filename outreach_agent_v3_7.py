"""
Semi-Agentic Outreach System v3.7 — Scoring Fix + People Search Fix
=====================================================
CHANGES FROM v3.6:
  [1] SCORING FIX — Revenue-aware zone scoring
      Problem: Core keywords are generic (miss most research text) while Beyond
      keywords are specific topic names (match too easily in F&B research).
      Result was Core=3, Beyond=10 for Nestlé when reality is Core=7, Beyond=3.
      Fix: Extract revenue % directly from research text and use it to SET the
      zone scores, rather than counting keyword matches. Keywords are now used
      as a confidence check, not the primary score.

  [2] PEOPLE SEARCH FIX — Robust parser + fallback + debug mode
      Problem: Claude ignores the ---PERSON--- format instruction and returns
      a numbered list. The parser finds 0 blocks and silently fails.
      Fix: Added fallback parser for numbered lists. Also increased max_tokens
      from 900 to 1400 to prevent truncation mid-response.

  [3] DEEPSEEK TRAINING OPT-OUT — header added to client
      deepseek_training_opt_out: true in config.yaml now actually sends the
      header to DeepSeek's API.

  [4] CLAUDE OPT-OUT — supported via config
      anthropic_opt_out: true in config.yaml noted (Anthropic API users are
      already opted out of training by default on the API).
...
THREE MODES:
  python outreach_agent_v3_7.py           → Monday: audit + hunt + plan
  python outreach_agent_v3_7.py --status  → Any day: quick check, no AI, no cost
  python outreach_agent_v3_7.py --friday  → Friday: upload JSON, CEO review
"""
...
def get_deepseek_client(config: dict):
    ...
    # [FIX v3.7] Actually send the training opt-out header if configured
    extra_headers = {}
...
# ─────────────────────────────────────────────────────────────
# [FIX v3.7] REVENUE-AWARE SCORING
# ─────────────────────────────────────────────────────────────

def extract_revenue_pct(text: str) -> dict:
    """
    Extract revenue percentage estimates from research text.
    The AI research says things like 'CORE (70-75%):' or 'EDGE (12-18%):'.
    We extract these and use them as the primary score, not keyword counts.

    Returns dict with core, edge, beyond as floats (midpoint of range).
    Returns None values if not found.
    """
    result = {"core": None, "edge": None, "beyond": None}

    # Patterns: "CORE (70-75%)" or "CORE: 70%" or "core business (70%)" etc.
    patterns = [
        # "CORE (70-75%)" or "CORE (70%)" or "BEYOND (5-10% but strategically deep)"
        (r'CORE[^(]*\((\d+)(?:[-–](\d+))?%[^)]*\)', "core"),
        (r'EDGE[^(]*\((\d+)(?:[-–](\d+))?%[^)]*\)', "edge"),
        (r'BEYOND[^(]*\((\d+)(?:[-–](\d+))?%[^)]*\)', "beyond"),
        # "70-75% core" or "~70% core"
        (r'~?(\d+)(?:[-–](\d+))?%[^)]*[Cc]ore', "core"),
        (r'~?(\d+)(?:[-–](\d+))?%[^)]*[Ee]dge', "edge"),
...
def revenue_pct_to_score(pct: float, zone: str) -> int:
    """
    Convert revenue % to a 1-10 score that reflects portfolio reality.

    Core: 70-75% revenue = score 7 (dominant but expected)
    Edge: 15-20% = score 5 (active but not dominant)
    Beyond: 5-10% = score 3 (present but small)

    The scale is intentionally non-linear:
    - Core 80%+ = 8-9 (over-indexed, disruption risk)
    - Core 60-80% = 6-7 (healthy dominant core)
    - Core <40% = 3-4 (core eroding)
    - Edge 20%+ = 7-8 (strong Edge investment)
    - Edge 10-20% = 4-6 (active Edge)
    - Edge <10% = 1-3 (weak Edge)
    - Beyond 10%+ = 6-7 (exceptional Beyond)
    - Beyond 5-10% = 3-4 (present Beyond)
    - Beyond 1-5% = 1-2 (minimal Beyond)
    - Beyond 0% = 0
    """
    if zone == "core":
        if pct >= 85: return 9
...
def theta_assess(research_text: str, company_name: str, industry: str,
    ...
    """
    [FIX v3.7] Revenue-aware scoring replaces pure keyword counting.

    Priority order:
    1. If research text contains revenue % estimates → use those as primary scores
    2. If no % found → fall back to keyword counting (old method)
    3. Case study text adds keyword signal as a bonus (max +1 per zone)

    This fixes the problem where:
    - Core scored 3 (few generic keywords) even though research said "70-75% revenue"
    - Beyond scored 10 (many specific topic names) even though research said "5-10% revenue"
    """
    ...
    # [FIX] Try to extract revenue % from research text first
    ...
    # Keyword signals (used as bonus/check)
    ...
    score_method = "revenue_pct"  # track which method was used

    if core_from_revenue is not None:
        # Revenue % found — use as primary, keyword signal adds max +1
        ...
        # Case study can add +1 to beyond if it has strong signals
        ...
        # No revenue % found — fall back to keyword counting with a cap
        # Cap beyond at 5 when using keyword-only method (prevents over-inflation)
        score_method = "keyword_count"
        core   = kw_core
        edge   = kw_edge
        beyond = min(5, kw_beyond)  # Keyword-only cap at 5 to prevent inflation
    ...
    return {
        ...
        "revenue_pcts_found":  {k: v for k, v in revenue_pcts.items() if v is not None},
    }
...
These percentages are critical for accurate scoring. Be specific — estimate based on
known revenue splits. For example:
  CORE (70-75%): Coffee, pet care, confectionery...
  EDGE (15-20%): Plant-based, DTC, digital transformation...
  BEYOND (5-10%): Microbiome research, cellular agriculture...
...
# ─────────────────────────────────────────────────────────────
# [FIX v3.7] FIND TARGET PEOPLE — Robust parser + fallback
# ─────────────────────────────────────────────────────────────

def parse_people_response(raw: str) -> list:
    """
    [FIX v3.7] Robust people parser with two strategies:
    1. Primary: look for ---PERSON--- blocks (structured format)
    2. Fallback: parse numbered lists (what Claude often returns instead)

    Also handles truncated responses gracefully.
    """
    people = []

    # Strategy 1: ---PERSON--- blocks (preferred)
    if "---PERSON---" in raw:
    ...
    # Matches patterns like "1. Stefan Palzer - Chief Technology Officer"
    # or "1. Name: Stefan Palzer / Title: CTO"
    ...
    for line in lines:
        ...
        # Detect numbered entry: "1." or "1)" at start
        ...
        if current is not None:
            # Try to extract title if not yet found
            ...
            # Try to extract LinkedIn search
...
def find_target_people(ds_client, claude_client, company: dict, research_bg: str,
    ...
    if cached_people:
        print(f"  ✓ People loaded from cache")
...
CRITICAL: You MUST use EXACTLY the format below. Do NOT use numbered lists.
Do NOT use any other format. Start each person with ---PERSON---
...
YOU MUST USE THIS EXACT FORMAT — no variations:
...
LINKEDIN_SEARCH: [Precise search string e.g. "{name}" "Chief Innovation Officer"]
...
Continue for all 3-5 people."""

    try:
        # [FIX v3.7] Increased max_tokens from 900 to 1400 to prevent truncation
        if ds_client:
            raw = call_deepseek(ds_client, prompt, max_tokens=1400)
        else:
            raw = call_claude(claude_client, prompt, max_tokens=1400)

        # [FIX v3.7] Use robust parser instead of inline parsing
        people = parse_people_response(raw)

        if not people:
            # Last resort: show raw response so we can debug
            print(f"  ⚠️  Parser found 0 people. Raw response preview:")
            print(f"  {raw[:300]}")
            print(f"  ...")
        ...
        if people:
            ...
            print(f"  ✓ {len(people)} people found via {model_name} — cached")
...
def build_contact_record(company: dict, theta: dict, chosen_variant: dict,
    ...
    return {
        ...
        "notes": (
            ...
            f"Scoring method: {theta.get('score_method','')}\n"
            f"Revenue %s: {theta.get('revenue_pcts_found','')}\n"
...
def save_all(approved_new, followup_actions, warm, patterns, cost):
    ...
    if approved_new:
        export = {
            ...
            "source":     "semi-agentic-outreach-v3.7",
            "note":       (
...
Board-meeting level. No fluff."""
    ...
    print(f"\n  📋  Next Monday: python outreach_agent_v3_7.py\n")
...
def main():
    parser = argparse.ArgumentParser(description="Semi-Agentic Outreach System v3.7")
    ...
    # ── MONDAY MODE ──────────────────────────────────────────
    banner("🧠  SEMI-AGENTIC OUTREACH v3.7  |  Theta Framework")
    print(f"  {datetime.now().strftime('%A, %B %d %Y  %H:%M')}")
    print(f"  {day_greeting()}")
    print(f"\n  v3.7 fixes active:")
    print(f"    ✓ Revenue-aware scoring — Core/Edge/Beyond reflect revenue reality")
    print(f"    ✓ People search fix — robust parser handles numbered lists")
    print(f"    ✓ DeepSeek training opt-out header — now actually sent")
    print(f"    ✓ Increased people lookup tokens — prevents truncation")
    ...
    if do_hunt == "y":
        ...
        if new_companies:
            ...
            print(f"  Your case studies will feed into the Theta scoring.\n")
            cs_contexts = {}
    ...
    print(f"  Any day → python outreach_agent_v3_7.py --status")
    print(f"  Fri  → Export tracker → python outreach_agent_v3_7.py --friday")
    print()
