"""
Semi-Agentic Outreach System v3.8 — Case Study Scoring + Verified People Cache
=====================================================
CHANGES FROM v3.7:

  [FIX 1] SCORING — Read % directly from case study HTML
      Problem: When a case study URL is provided, the script fetched the HTML
      but then ignored the actual allocation numbers and asked DeepSeek to
      guess the Core/Edge/Beyond split. DeepSeek over-weighted Beyond because
      Beyond topics have specific vocabulary that scores high on keyword counting.
      Fix: extract_pct_from_case_study() reads percentages directly from Pam's
      case study HTML. These are ground truth — no API call needed for scoring.
      Priority chain:
        1. Case study HTML provided → extract % from it → use those scores
        2. No case study → DeepSeek research text → extract % from CORE (XX-XX%) format
        3. Neither found → keyword fallback (Beyond capped at 5)

  [FIX 2] PEOPLE — Verified cache in learnings.json + honest confidence flags
      Problem: DeepSeek API has no internet access. It guesses from training data.
      Names are often wrong (e.g. returned "Bérangère Magarinos-Ruchat" for Danone
      — does not appear on current Danone leadership pages). This is a fundamental
      limitation, not a bug.
      Fix (Part A): verified_people key in learnings.json. Cache lookup runs before
      any API call. Cache expires after 90 days.
      Fix (Part B): API-sourced names now show a clear LOW CONFIDENCE warning.
      Verified cache names show a VERIFIED banner with source and age.
      New: [m] manual entry option — Pam can feed in names she found on LinkedIn
      and save them directly to the verified cache.
      Pre-populated: Danone leadership verified 2026-03-12 from danone.com/governance.
...
THREE MODES:
  python outreach_agent_v3_8.py           → Monday: audit + hunt + plan
  python outreach_agent_v3_8.py --status  → Any day: quick check, no AI, no cost
  python outreach_agent_v3_8.py --friday  → Friday: upload JSON, CEO review
"""
...
def get_deepseek_client(config: dict):
    ...
    NOTE: DeepSeek API has NO live internet access. Names from it are training-data
    guesses and must be treated as LOW CONFIDENCE until verified.
    """
...
def load_learnings() -> dict:
    ...
    return {
        ...
        "last_tracker_file":     "",
        "verified_people":       {},   # [v3.8] new top-level key
    }
    ...
    if has_cs == "y":
        ...
        if raw.startswith("http"):
            ...
            try:
                ...
                # [v3.8] Store raw HTML for case study extraction, plus stripped text
                context["case_study_html"] = html[:8000]   # raw HTML for pct extraction
                text = re.sub(r"<[^>]+>", " ", html)
                text = re.sub(r"\s+", " ", text).strip()
                context["case_study_text"] = text[:4000]
                print(f"  ✓ Case study fetched ({len(context['case_study_text'])} chars)")
                # [v3.8] Try extracting pcts immediately so we can tell Pam
                pcts = extract_pct_from_case_study(html)
                if any(v is not None for v in pcts.values()):
                    core_s   = f"{pcts['core']:.0f}%"   if pcts['core']   is not None else "not found"
                    edge_s   = f"{pcts['edge']:.0f}%"   if pcts['edge']   is not None else "not found"
                    beyond_s = f"{pcts['beyond']:.0f}%" if pcts['beyond'] is not None else "not found"
                    print(f"  📎  Zone % extracted: Core {core_s} | Edge {edge_s} | Beyond {beyond_s}")
                    print(f"     These will be used directly for scoring (no API guess needed)")
                else:
                    print(f"  ℹ️  No zone % found in case study HTML — will use DeepSeek research text")
            except Exception as e:
...
def find_target_people(ds_client, claude_client, company: dict, research_bg: str,
                       cost: CostTracker, learnings: dict) -> tuple:
    """
    [v3.8] Returns (people_list, source) where source is one of:
      'verified_cache' — from learnings.json verified_people (show green banner)
      'api'            — from DeepSeek / Claude (show red warning)
      'manual'         — manually entered by Pam (show green banner)

    Priority:
      1. verified_people in learnings.json (90-day TTL)
      2. DeepSeek / Claude API (with LOW CONFIDENCE warning)
    """
    name = company.get("name","")

    # ── PRIORITY 1: Verified cache ────────────────────────────
    ...
    # ── PRIORITY 2: Old-style per-company cache (people TTL=30d) ─
    cached_people = load_cached_people(name)
    if cached_people:
        print(f"  ✓ People loaded from file cache (unverified)")
        cost.charge("cached_people", note=f"{name} people (file cache)")
        return cached_people, "api", None   # treat file cache as API-confidence
...
- If you do NOT know, say "Search required" — do NOT invent names
- Rank by likelihood they own portfolio-level innovation
...
Continue for all 3-5 people."""
    ...
    try:
        ...
        if people:
            ...
            print(f"  ✓ {len(people)} people found via {model_name} — file-cached")
...
def people_selection_loop(people: list, source: str, verified_entry: dict | None,
    ...
    """
    if source == "verified_cache" and verified_entry:
...
def main():
    parser = argparse.ArgumentParser(description="Semi-Agentic Outreach System v3.8")
    ...
    # Ensure verified_people key always exists (upgrade from older learnings.json)
    if "verified_people" not in learnings:
        learnings["verified_people"] = {}
    ...
    # ── MONDAY MODE ──────────────────────────────────────────
    banner("🧠  SEMI-AGENTIC OUTREACH v3.8  |  Theta Framework")
    print(f"  {datetime.now().strftime('%A, %B %d %Y  %H:%M')}")
    print(f"  {day_greeting()}")
    print(f"\n  v3.8 fixes active:")
    print(f"    ✓ [FIX 1] Case study scoring — zone % read from your HTML, not guessed by DeepSeek")
    print(f"    ✓ [FIX 2] Verified people cache — learnings.json stores confirmed names")
    print(f"    ✓ [FIX 2] Honest confidence flags — LOW/VERIFIED banners on all people lists")
    print(f"    ✓ [FIX 2] Manual entry [m] — feed in LinkedIn-verified names and cache them")
    print(f"    ✓ Danone verified people pre-loaded (danone.com/governance, 2026-03-12)")
    ...
    if do_hunt == "y":
        ...
        if new_companies:
            ...
            for i, company in enumerate(new_companies, 1):
                ...
                # [v3.8] Pass learnings so verified cache can be checked
                ...
                if not people:
                    ...
                    print("  You can still enter someone manually.")
                ...
                # [v3.8] New people display + selection loop with confidence banners
                ...
                for person in selected_people:
                    ...
                    if theta.get("case_study_source") == "case_study_html":
                        ...
                        print(f"  📎  Scores sourced directly from your case study "
                              f"(Core: {core_s}, Edge: {edge_s}, Beyond: {bey_s})")
                    elif theta.get("case_study_informed"):
    ...
    print(f"  Any day → python outreach_agent_v3_8.py --status")
    print(f"  Fri  → Export tracker → python outreach_agent_v3_8.py --friday")
    print()
