"""
Semi-Agentic Outreach System v3.4 — Theta-Native Research + People-First Outreach
=====================================================
...
def display_audit(urgent, warm, stale, patterns):
    ...
    for i, c in enumerate(urgent, 1):
        ...
        # Drop signal: no response, or vague curiosity with no decision power
        resp_lower = resp.lower()
        touches = len(c.get("communicationLog",[]))
        is_no_response = not resp.strip() or resp_lower in ["no response","none",""]
        is_weak_response = any(phrase in resp_lower for phrase in [
            "not part of", "not in", "just curious", "not my area", "not my team"
        ])
        if is_no_response:
            print(f"     ⚠️  No real response — consider dropping (skip reply)")
        elif is_weak_response and touches >= 2:
            print(f"     ⚠️  Weak signal (not decision-maker, low engagement) — consider dropping")
    ...
    # Build a map of warm contacts by company for cross-referencing
    warm_by_company = {}
    for w in warm:
        co = (w.get("company","") or "").strip().lower()
        if co:
            warm_by_company.setdefault(co, []).append(w)
    ...
    for c in sorted(stale, key=lambda x: x.get("_days_ago",0), reverse=True)[:5]:
        ...
        if co_key in warm_by_company:
            alts = warm_by_company[co_key]
            alt_names = ", ".join(a.get("name","") for a in alts[:2])
...
optimizing their core business and building next-generation growth. Geography is a preference, not a hard filter — if fewer than {target_count} strong matches exist in that region, include the best global alternatives.
...
You MUST output exactly {target_count} companies. Do not output fewer.

Output EXACTLY this format for each (copy the delimiters exactly):
...
THE THETA FRAMEWORK — Core Definitions (use these exactly):

ZONES:
- 🟩 CORE: Incremental improvements to existing products/markets. Optimizes current business.
  Types: Core (existing tech, existing market), Adjacent (existing capabilities → new markets)
- 🟨 EDGE: Next S-curve bets. High tech change or new market. Not yet mainstream.
  Types: Architectural (reconfiguring components), Disruptive (starts small, displaces incumbents)
- 🟥 BEYOND: Speculative, long-horizon. Transformational or Frontier R&D. 7–15+ year horizon.
  Types: Transformational (industry-shifting), Frontier (quantum, biotech, speculative)

ARCHETYPES:
- Balanced Transformer: Active Core + Edge + Beyond (e.g. NVIDIA, Block)
- Disciplined Explorer: Strong Core, active Edge, funded Beyond (e.g. Nestlé)
- Legacy-Rich Vision-Poor: Core ✓, Edge weak, Beyond absent (e.g. IBM)
- Profitable Stagnant: Core ✓, Edge ✗, Beyond ✗ (e.g. Oracle)
- Rebel Innovator: Edge-heavy, Beyond-bold, Core-challenged (early Tesla)
- Innovation Theater: Labs and pilots, but nothing ships to scale
- Frontier Builder: Beyond bets without Core/Edge bridge

KEY PRINCIPLE: "A healthy innovation portfolio spans all three zones. Companies that innovate
only in Core optimize themselves into irrelevance." Beyond ≠ moonshots only — it includes
any transformational or frontier bet with a 7–15+ year horizon.
"""

def research_company(client, company: dict, cost: CostTracker, cs_context: dict = None) -> dict:
    ...
    if "background" in cached:
        ...
        # Build case study context for the prompt
        cs_insert = ""
        if cs_context:
            if cs_context.get("case_study_text"):
                cs_insert = f"\n\nPAM'S CASE STUDY CONTENT (read this carefully):\n{cs_context['case_study_text'][:3000]}"
            ...
            if cs_context.get("article_texts"):
                cs_insert += f"\n\nPam's article excerpt: {cs_context['article_texts'][0][:1500]}"
...
Provide:
1. CORE zone: What are their main Core initiatives? (existing products, efficiency, optimization)
2. EDGE zone: What are their active Edge bets? (new platforms, adjacent markets, digital transformation)
3. BEYOND zone: What are their Beyond investments? (transformational R&D, long-horizon bets, frontier tech) — be thorough, do NOT miss these. For companies like Nestlé, there are significant Beyond investments (plant-based, gut microbiome, precision nutrition, etc.)
4. ARCHETYPE: Which of the 7 archetypes fits best, and why?
5. BIGGEST GAP: Where is their portfolio weakest relative to Theta balance?
6. WHO OWNS BREAKTHROUGH: Which specific role/department owns Edge and Beyond innovation at this company? (e.g. Chief Innovation Officer, Head of Innovation Portfolio, VP R&D Ventures)

Under 350 words. Be specific — name actual programs and initiatives."""
        try:
...
def ask_case_study_context(company_name: str, industry: str) -> dict:
    """Ask Pam for her case study / articles. If a URL is given, fetch and summarise it."""
    ...
    if has_cs == "y":
        ...
        if raw.startswith("http"):
            ...
            try:
                ...
                print(f"  ✓ Case study fetched ({len(context['case_study_text'])} chars)")
            except Exception as e:
...
# ─────────────────────────────────────────────────────────────
# STEP 4b — FIND TARGET PEOPLE (who owns Edge/Beyond)
# ─────────────────────────────────────────────────────────────

def find_target_people(client, company: dict, research_bg: str, cost: CostTracker) -> list:
    """Find 3–5 specific roles/names who own breakthrough innovation at this company."""
    name     = company.get("name","")
    industry = company.get("industry","")
...
Identify 3–5 specific people who are MOST LIKELY to own Edge and Beyond innovation at {name}.

For each person:
- If you know their actual name and current role, provide it
- If you only know the role type, provide the role and a LinkedIn search string
- Rank them by how likely they are to be the right Theta conversation partner
- Note WHY they're the right person (what they own: portfolio governance, R&D ventures, digital transformation, etc.)

Format EXACTLY:

---PERSON---
RANK: [1-5]
NAME: [Full name if known, or "Unknown"]
TITLE: [Exact current title]
WHY: [One sentence — what they own that's relevant to Theta]
LINKEDIN_SEARCH: ["Nestlé" "Chief Innovation Officer" site:linkedin.com]
CONFIDENCE: [High = known name / Medium = likely title / Low = guessing]
...
Repeat for each person."""
    ...
    try:
        ...
        for block in raw.split("---PERSON---")[1:]:
            ...
            if p.get("title"):
                people.append(p)
        cost.charge("research", note=f"{name} people")
        return people[:5]
    except Exception as e:
        return []


def check_public_channels(client, person: dict, company_name: str) -> dict:
    """For a given person, determine which public channel is easiest first touch."""
    name  = person.get("name","Unknown")
    title = person.get("title","")

    if name == "Unknown":
        # No known name, LinkedIn search string is only option
        return {
            "recommended": "LinkedIn",
            "rationale": "No confirmed name — use LinkedIn search string to find and connect first",
            "connection_note": "LinkedIn connect required before messaging",
            "channels": {"LinkedIn": "Search → connect → message after accepted"}
        }

    prompt = f"""For {name}, {title} at {company_name}:

Do a quick public channel assessment. Based on what you know:
1. Are they active on LinkedIn? (post regularly, comment publicly)
2. Do they have a public Twitter/X account?
3. Do they write on Substack or personal blog?
4. Is their email findable publicly (company format or personal)?
5. Have they spoken at conferences (likely on speaker pages)?

For each channel: Exists (likely/unlikely/unknown) and ease of first contact (Easy/Medium/Hard).

Then recommend: which channel gives the EASIEST warm first touch?
...
# ─────────────────────────────────────────────────────────────
# STEP 5b — DRAFT CONNECTION + POST-CONNECTION MESSAGES
# ─────────────────────────────────────────────────────────────

def draft_connection_and_message(client, company: dict, person: dict, theta: dict,
                                  channel_info: dict, cs_context: dict,
                                  config: dict, patterns: dict,
                                  linkedin_notes_available: bool = True) -> dict:
    """Draft: (a) LinkedIn connection note if available, (b) post-connection first message."""
    user_name    = config.get("user_short_name","Pam")
    ...
    content_hook = ""
    if cs_context.get("case_study_text"):
        content_hook = f"Pam has a detailed case study on {company_name} covering their Core/Edge/Beyond portfolio. Key content: {cs_context['case_study_text'][:500]}"
    elif cs_context.get("case_study_note"):
...
---CONNECTION NOTE--- (LinkedIn invite, max 300 chars — this is the hard character limit)
[message — must be under 300 characters]

---POST CONNECTION MESSAGE--- (send after they accept — can assume they saw the connection note)
CHANNEL: {channel}
TONE: [label]
BODY:
[message]""".format(channel=recommended)
        extra_rule = "- Connection note: ONE sentence hook, curiosity-only, no ask, under 300 chars"
    else:
        note_instruction = """IMPORTANT: Pam has NO connection notes available (free LinkedIn limit reached).
She will send a BLANK invite. The message below is the FIRST thing this person ever reads from her.
It must be warm enough to stand alone without any prior context.

---POST CONNECTION MESSAGE--- (first message after blank invite accepted)
CHANNEL: {channel}
TONE: [label]
BODY:
[message]""".format(channel=recommended)
        extra_rule = "- No connection note was sent — message must be self-contained and warm, not assume prior context"

    prompt = f"""You are drafting outreach for {config.get('user_name','Christine Pamela')}, an innovation consultant (Theta Framework).

TARGET: {person_name} — {title} at {company_name}
WHY THEM: {why}
...
- If Pam has a case study on this exact company, the post-connection message can reference it naturally
- Sign as "{user_name}"
    ...
    try:
        ...
        for line in raw.splitlines():
            ...
            if "---POST CONNECTION MESSAGE---" in s:
                in_post = True
                continue
            if not in_post and s and "---" not in s and "BODY:" not in s and "CHANNEL:" not in s and "TONE:" not in s:
                connection_note += s + " "
            if in_post and s.startswith("BODY:"):
                continue
            if in_post and not s.startswith("CHANNEL:") and not s.startswith("TONE:") and "---" not in s:
                post_msg += line + "\n"
        ...
        fallback_note = f"Hi — I've been researching {company_name}'s innovation portfolio and would value your perspective. Happy to connect." if linkedin_notes_available else ""
        fallback_msg  = f"Hi — thanks for connecting. I've been studying {company_name}'s innovation portfolio and had a few questions I thought you might find interesting. Would you be open to a quick conversation?\n\nBest,\n{user_name}"
        return {
...
def approval_loop_new(company: dict, theta: dict, research: dict,
    ...
    if choice == "e":
        print("  Paste message (blank line to finish):")
        lines = []
        while True:
            line = input()
            if line == "":
...
def main():
    parser = argparse.ArgumentParser(description="Semi-Agentic Outreach System v3.3")
    ...
    # ── MONDAY MODE ────────────────────────────────────────────
    banner("🧠  SEMI-AGENTIC OUTREACH v3.4  |  Theta Framework")
    ...
    if do_hunt == "y":
        # Ask once per session about LinkedIn note availability
        print("\n  LinkedIn connection note status:")
        print("  (Free LinkedIn caps how many invites can include a note)")
        ...
        if new_companies:
            print(f"\n  Reviewing {len(new_companies)} companies. One at a time.")
            ...
            for i, company in enumerate(new_companies, 1):
                ...
                # STEP A: Ask for Pam's case study / articles FIRST (so research can use them)
                cs_context = ask_case_study_context(cname, company.get("industry",""))

                # STEP B: Theta brief — now informed by case study content
                print(f"\n  Analysing {cname} with Theta Framework...")
                research = research_company(client, company, cost, cs_context)
                theta    = research["theta"]
                cached_tag = " (cached)" if research.get("was_cached") else ""

                # Display Theta brief
                section(f"THETA BRIEF — {cname}")
                print(f"  {company.get('industry','')} | {company.get('country','')} | ~{company.get('size','?')} employees")
                print(f"\n  Archetype:  {theta.get('archetype','')}")
                print(f"  Zones:      Core={theta['zone_distribution']['core']}  Edge={theta['zone_distribution']['edge']}  Beyond={theta['zone_distribution']['beyond']}")
                print(f"  Pain:       {theta.get('pain_point','')}")
                print(f"  Best angle: {theta.get('messaging_angle','')}")
                print(f"\n  Research{cached_tag}:")
                for line in research.get("background","").splitlines():
                    if line.strip():
                        print(f"    {line}")

                # Allow archetype correction before proceeding
                correct_arch = ask("\n  Archetype correct?", ["y","n"])
                if correct_arch == "n":
                    for j, a in enumerate(VALID_ARCHETYPES, 1):
                        print(f"    {j}. {a}")
                    raw = input("  Correct archetype number: ").strip()
                    if raw.isdigit() and 1 <= int(raw) <= len(VALID_ARCHETYPES):
                        correct = VALID_ARCHETYPES[int(raw)-1]
                        theta.update(theta_assess(research.get("background",""), cname,
                                                  company.get("industry",""), override_archetype=correct))
                        corrections = learnings.get("archetype_corrections", {})
                        corrections[cname] = {"was": theta.get("archetype"), "corrected_to": correct,
                                               "date": datetime.now().isoformat()}
                        learnings["archetype_corrections"] = corrections
                        save_learnings(learnings)
                        print(f"  ✓ Corrected to: {correct}")
                ...
                # STEP C: Find target people
                print(f"\n  Finding who owns breakthrough innovation at {cname}...")
                people = find_target_people(client, company, research["background"], cost)
                ...
                for p in people:
                    ...
                    print(f"\n  [{p['rank']}] {p['name']} — {p['title']}")
                    print(f"       Why: {p['why']}")
                    print(f"       Confidence: {conf}")
                    if p.get("linkedin_search"):
                        print(f"       LinkedIn: {p['linkedin_search']}")

                # Let Pam pick which person to target
                print()
                ...
                # STEP D: For each selected person — check channels + draft messages
                for person in selected_people:
                    pname = person.get("name","Unknown")
                    print(f"\n  Checking public channels for {pname}...")
                    channel_info = check_public_channels(client, person, cname)
                    cost.charge("theta", note=f"channels {pname}")

                    print(f"\n  Channel assessment for {pname}:")
                    ...
                    print(f"\n  Drafting messages for {pname}...")
                    drafts = draft_connection_and_message(
                        client, company, person, theta, channel_info,
                        cs_context, config, patterns,
                        linkedin_notes_available=linkedin_notes_available)
                    cost.charge("draft", note=pname)

                    # Display and approval
                    section(f"OUTREACH DRAFTS — {pname} @ {cname}")
                    ...
                    if drafts.get("connection_note") and linkedin_notes_available:
                        ...
                        print(f"\n  ── B) POST-CONNECTION MESSAGE (send after they accept) ──")
                    else:
                        if not linkedin_notes_available:
                            print(f"\n  ℹ️  No connection note (limit reached) — blank invite.")
                            print(f"  The message below is your FIRST impression (send after they accept):\n")
                        else:
                            print(f"\n  ── FIRST MESSAGE (send after they accept) ──")
                    ...
                    print()
                    if linkedin_notes_available:
                        choice = input("  [a=approve both / p=post-msg only / e=edit post-msg / n=skip]: ").strip().lower()
                    else:
                        choice = input("  [a=approve message / e=edit / n=skip]: ").strip().lower()
                        if choice == "a":
                            choice = "p"  # normalise — no connection note to save
                    ...
                    if choice in ["a", "p"]:
                        save_connection_note = (choice == "a" and linkedin_notes_available
                                                and drafts.get("connection_note"))
                        record = build_contact_record(company, theta,
                                                      {"variant":1, "channel": drafts["channel"],
                                                       "tone":"connection", "subject":"",
                                                       "body": drafts["post_connection"]},
                                                      cs_context)
                        note_parts = []
                        if save_connection_note:
                            note_parts.append(f"LinkedIn connection note: {drafts['connection_note']}\n")
                        note_parts.append(f"First message (post-connection):\n{drafts['post_connection']}")
                        record["notes"]    = "\n".join(note_parts)
                        record["name"]     = person.get("name","") if person.get("name") != "Unknown" else ""
                        record["jobTitle"] = person.get("title","")
                        record["_linkedin_search"] = person.get("linkedin_search","")
                        record["_has_connection_note"] = save_connection_note
                        approved_new.append(record)
                        status_msg = "connection note + message" if save_connection_note else "message only (blank invite)"
                        print(f"  ✓ Added: {pname} — {status_msg}")

                        # Log message edit as learning
                        if choice == "e":
                            edits = learnings.get("message_edits",[])
                            edits.append({
                                "company": cname, "person": pname,
                                "original": drafts["post_connection"],
                                "edited": drafts["post_connection"],
                                "date": datetime.now().isoformat()
                            })
